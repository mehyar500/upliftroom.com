...
async function checkRateLimit(supabase, ip) {
  ...
  const { data, error } = await supabase.from("daily_users").upsert(
    {
      ip_address: ip,
      visit_date: today,
      request_count: 1,
  ...
  ).select("request_count").single();
  if (error) {
    const { data: existing, error: selectError } = await supabase.from("daily_users").select("request_count").eq("ip_address", ip).eq("visit_date", today).single();
    ...
    await supabase.from("daily_users").update({
      request_count: existing.request_count + 1,
      last_seen: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("ip_address", ip).eq("visit_date", today);
  ...
  const { data: updated } = await supabase.from("daily_users").update({
    request_count: (data?.request_count || 0) + 1,
    last_seen: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("ip_address", ip).eq("visit_date", today).select("request_count").single();
...
async function handleAdminLogin(request, env) {
  ...
  const supabase = getSupabaseClient(env);
  const { data, error } = await supabase.from("admins").select("username").eq("username", username).eq("password", password).maybeSingle();
  if (error) {
    return jsonResponse(
...
async function getProduct(slug, env) {
  const supabase = getSupabaseClient(env);
  const { data, error } = await supabase.from("products").select("*, categories(name, slug)").eq("slug", slug).eq("is_active", true).single();
  if (error || !data) {
    return jsonResponse(
      { status: "error", message: "Product not found" },
      404
...
async function updateProduct(id, request, env) {
  ...
  if (payload.is_active === true) {
    ...
    const { data: existing } = await supabase2.from("products").select("image_cover_path, profile, intensity, short_description").eq("id", id).single();
    if (!existing) {
...
async function uploadImage(request, env) {
  try {
    ...
    const { data, error } = await supabase.storage.from("product-images").upload(filename, arrayBuffer, {
...
async function deleteImage(path, env) {
  const supabase = getSupabaseClient(env);
...
// src/routes/rss.ts
async function getRSSItems(request, env) {
  const url = new URL(request.url);
  const limit = parseInt(url.searchParams.get("limit") || "20");
  const category = url.searchParams.get("category");
  const supabase = getSupabaseClient(env);
  let query = supabase.from("rss_items").select("*, rss_sources(name, homepage_url, category)").order("published_at", { ascending: false }).limit(limit);
  if (category) {
...
__name(getRSSItems, "getRSSItems");
async function fetchRSS(env) {
  const supabase = getSupabaseClient(env);
  const { data: sources, error: sourcesError } = await supabase.from("rss_sources").select("*").eq("is_active", true);
    ...
    try {
      ...
      for (const item of items) {
        const { error: insertError } = await supabase.from("rss_items").upsert({
          ...
          published_at: item.pubDate ? new Date(item.pubDate).toISOString() : (/* @__PURE__ */ new Date()).toISOString()
        }, {
          onConflict: "link",
      ...
      await supabase.from("rss_sources").update({ last_fetched_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", source.id);
      results.push({
        source: source.name,
        status: "success",
        inserted,
  ...
  for (const match of matches) {
    ...
    if (title && link) {
      items.push({
        ...
        link: cleanText(link),
        description: description ? cleanText(description) : void 0,
        content: content ? cleanText(content) : void 0,
        pubDate: pubDate ? cleanText(pubDate) : void 0,
        author: author ? cleanText(author) : void 0
      });
    }
  }
...
function cleanText(text) {
  return text.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/<[^>]+>/g, "").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&#39;/g, "'").trim();
}
